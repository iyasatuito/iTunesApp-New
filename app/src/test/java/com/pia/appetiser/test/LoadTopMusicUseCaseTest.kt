...
class LoadTopMusicUseCaseTest {


    lateinit var useCase: LoadTopMusicUseCase
    ...
    fun setUp() {
        ...
        useCase = LoadTopMusicUseCase(repository)
    }

    @Test
    fun `Given_top_music_are_not_null_when_fetched_then_results_has_data`() {
        val movies = generateDummyMovies()
        val mappedMovies = movies.map { it.toDisplayableItunesDetails() }
        val values = Single.just(movies)

        every { repository.getTopMusic() } returns values
        useCase()
            .test()
        ...
        verify(exactly = 1) {repository.getTopMusic()}
