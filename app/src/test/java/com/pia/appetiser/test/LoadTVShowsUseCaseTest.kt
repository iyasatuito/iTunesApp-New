...
import com.pia.appetiser.test.domain.appusecase.LoadTVShowsUseCase
...
class LoadTVShowsUseCaseTest {

    lateinit var useCase: LoadTVShowsUseCase
    ...
    fun setUp() {
        ...
        useCase = LoadTVShowsUseCase(repository)
    }

    @Test
    fun `Given_tv_shows_are_not_null_when_fetched_then_results_has_data`() {
        val movies = generateDummyMovies()
        val mappedMovies = movies.map { it.toDisplayableItunesDetails() }
        val values = Single.just(movies)

        every { repository.getTVShows() } returns values
        ...
        verify(exactly = 1) {repository.getTVShows()}
